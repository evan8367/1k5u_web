...
public final class HttpClientUtil {
	...
	public synchronized static HttpClient init() {
		try {
			...
			// 设置访问协议
			Registry<ConnectionSocketFactory> schreg = RegistryBuilder
	...
	public static String postData(String url,
			List<FormBodyPart> multipartformData, String charset) {
	 ...
	 */
	public static String postData(String url, String data,
			Map<String, String> formData, List<FormBodyPart> multipartformData,
		...
		try {
			...
			// 发送请求
			HttpResponse response = getResponse(httpClient, url, data,
					formData, multipartformData, headers, cookie, contentType,
					null);
			String result = readResponse(response.getEntity(), charset);
			// 打印日志
			log(url, data, formData, result, beginTime);
	 ...
	 * @return
	 */
	private static HttpResponse getResponse(HttpClient httpclient, String url,
			String data, Map<String, String> formData,
			List<FormBodyPart> multipartFormData, Map<String, String> headers,
			String cookie, ContentType contentType, HttpContext localContext)
			throws Exception {
		HttpPost httppost = new HttpPost(url);
		if (data != null) {
			StringEntity reqEntity = null;
			if (contentType == null) {
				reqEntity = new StringEntity(data, ContentType.create(
	...
	private static void log(String url, String data,
			Map<String, String> formData, String result, long beginTime) {
		if (logger.isDebugEnabled()) {
			...
			logger.debug(new StringBuffer().append("请求地址: ").append(url)
					.append("\n 请求参数: ").append(data).append("\n 响应时间：")
					.append(time).append("毫秒").append("\n 响应结果：")
